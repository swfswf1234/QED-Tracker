...
关联代码：`src/qed_tracker/application/migrate_knowledge.py`（存量迁移命名）、
`src/qed_tracker/cli.py`（新建知识行默认命名）、`src/qed_tracker/db/knowledge_repository.py`
关联测试：`tests/test_migrate_knowledge.py`、`tests/test_cli_architecture.py`、
...
- `tests/test_migrate_knowledge.py` 新增/更新断言：存量迁移命名符合规范格式；
  `tests/test_cli_architecture.py` 断言新建知识行默认命名；`tests/test_knowledge_api.py`
  断言 API 返回的 name 已规范。
- 真实 MySQL 冒烟：01 数学分析 3 行 `name` 分别为
  `教程1：数学分析（Rudin）`、`教程2：微积分学教程（菲赫金哥尔茨）`、`教程3：数学分析（陈纪修）`
  （以实际书行 authors/title 为准）。
- 回执根仓库 REQ-041（提交号 + 测试输出），根仓库前端左树教程叶子联调验收。