# 来源探索与评估计划（source-discovery）

状态：Draft
任务类型：B
最后更新：2026-09-07
需求方：QED-Tracker（来源持续探索，QED-018 已关闭后的延续工作）
目标项目：QED-Tracker
评审方：用户

> 本文档 2026-09-07 自 `docs/design/source-discovery.md` 移入 plans（ADR 0008，design→plans
> 反向流转首例）：设计契约部分（来源协议边界、libgen_li 发现专用裁决）已并入
> [下载管线设计](../design/download-pipeline.md)，本计划承载**持续探索工作**——来源评估
> 矩阵、实测记录与待探索清单。
