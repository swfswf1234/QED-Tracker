...
## REQ-020 承接（2026-09-09 自下载流程计划迁入，原计划已关闭删除）

### REQ-020 ② 找得率榜单（承接口径）

根仓库 REQ-020② 要求"各来源渠道命中率/下载成功率实测统计，回填 QED-Tracker source-discovery 矩阵"。

1. **口径定义**：「找得率」= 各渠道搜索返回有效候选的占比（对齐本文档矩阵的「候选质量/下载成功率」列）。
2. **数据源**：`qt_sources` 表的渠道尝试记录（每次 search/download 一条 source 行；汇总视图 `mainline channels`）。
3. **执行时机**：主链路 00/01/02 三门真实环境取书闭环已通过（QED-014 验收，2026-09-09）——基线数据具备采集条件，采集与回执随 QED-054 持续工作执行。
4. **回执方式**：基线数据产出后回执根仓库，标注"口径已定义、基线已采集"；不单独建界面，产出为统计数字。

### REQ-020 ① 权威性榜单（收窄建议）

根仓库 REQ-020① 要求"各教材/版本在数学社区权威性排序，服务第一轮评估选书"。

- 现行承载：权威性评价由 LLM 预填产出（`mainline-prefill-v1` 契约 `authority` ∈ 高/中/低 +
  防总评高校准，见[主链路设计](../design/main-line-curriculum.md) §5），经人工评审定稿。
- **收窄建议**：收窄为"随课程扩展沉淀"，不单独立项；知会根仓库调整范围。

## 执行记录

- 2026-09-09：REQ-020①② 承接声明自 `2026-08-download-flow.md`（本轮关闭删除）迁入本计划
  （找得率口径 + 权威性榜单收窄建议）；golden 范本文件已随 QED-050 重构移除，原表述不再保留。
