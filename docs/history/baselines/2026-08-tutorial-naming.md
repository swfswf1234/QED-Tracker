...
归档说明：
- 范围：`tutorial_name` 命名规则（`教程{set_no}：{首作者}《{书名}》`等）、mainline
  new/review 命名路径、前端原样展示边界、决策登记与 QED-036 回执。
- 失效原因：与知识录入同域，按 ADR 0008 整篇并入
  [知识录入设计](../../design/knowledge-import.md)「教程命名规范」节，本文退役。
- 不可变锚点：正文以 2026-09-07 归档时点工作树状态冻结（本批文档含未提交修订，随本轮
  提交定稿；此后仅允许补 Historical 声明、反向关系或修复链接）。
- 当前事实入口：[知识录入设计](../../design/knowledge-import.md)。
设计状态：Historical（2026-09-07 归档）
实现状态：Completed（承载内容已并入 knowledge-import.md；证据链见
[QED-036 证据目录](../qed-036-tutorial-naming/index.md)）
确认状态：已确认
最后更新：2026-09-07
需求方：QED-Engine（根仓库 REQ-041「教程命名规范」，设计依据根仓库
[downloads-manage-redesign.md](../../../docs/design/downloads-manage-redesign.md) §2.3 用户裁决）
关联代码：`src/qed_tracker/db/knowledge_repository.py`（`tutorial_name` 命名函数 + `adopt_tutorials`
先查后建幂等）、`src/qed_tracker/cli.py`（mainline new 默认命名与决定引用）
关联测试：`tests/test_knowledge_repository.py`、`tests/test_main_line_cli.py`、
...
>    重放不产生重复行），存量改名由一次性数据修正脚本负责。幂等键现由 `adopt_tutorials`
>    承接；`migrate` 命令与存量迁移脚本已随旧三表路径退役删除（QED-050-D Phase 6）。
...
2. **新建教程**（`cli.py` 的 mainline new）直接 `name=args.title`，未按套规范生成；
...
- **方案 A（推荐）**：`textbook_ref` 扩展为 `{title, version, authors}`——决定引用补
  ...
  教程自含命名所需信息；需同步 `database-private-tables.md` 引用数组说明与迁移时回填 authors。
- **方案 B**：命名时从该套**教材书籍**（`qt_books` 中 `roles` 含 textbook 的首行）聚合
  `title` + `authors`。不改决定引用结构，但命名依赖书籍存在性（draft 期书籍可能未定）。
- **方案 C**：仅 `教程{set_no}：{书名}`，不显示作者。实现最简，但不符合根仓库裁决的
  「书名（作者）」格式。

推荐 **方案 A**：决定引用补齐 `authors` 后，命名规则成为纯函数（只依赖教程自身），
draft 期即可生成规范名；存量 01 数学分析 3 行已有教材书籍 authors 可回填。

## 改动范围

1. **命名默认值生成**：
   - `src/qed_tracker/db/knowledge_repository.py`：采纳路径 name 生成
     `教程{set_no}：{首作者}《{书名}》`（tutorials@v2 格式），经 `adopt_tutorials` 先查后建
     幂等落行（原 migrate 循环内生成已随存量迁移脚本退役删除，QED-050-D Phase 6）。
   - `src/qed_tracker/cli.py`（mainline new 新建教程）name 默认值：有 set_no 时
     按规范生成，否则保持 title（draft 期命名，人工可改名）。
2. **textbook_ref 结构**（方案 A）：`database-private-tables.md` 引用数组说明补充 authors；
...
- `tests/test_knowledge_repository.py` 断言采纳路径命名符合规范格式 + textbook_ref 回填
  authors + 先查后建幂等复用（存量迁移命名测试文件随存量迁移脚本退役删除，
  QED-050-D Phase 6）；`tests/test_main_line_cli.py` 断言
  mainline new `--set-no` 规范名与决定引用；`tests/test_knowledge_api.py` 断言规范名经 API 透出。
- 真实 MySQL 冒烟（2026-08-20）：01 数学分析 3 行 `name` 修正为
  `教程1：数学分析原理（Rudin）`、`教程2：微积分学教程（菲赫金哥尔茨）`、
  `教程3：数学分析（陈纪修）`（套 3 书籍 title 已含「（陈纪修）」由命名函数去重）；
