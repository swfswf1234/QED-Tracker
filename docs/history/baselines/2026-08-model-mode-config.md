...
归档说明：
- 范围：`.env` 密钥分置与读取优先级、模型模式（local/qed-engine）与 llm_client 兼容层、
  `--mode` 服务接入、决策登记（QED-037/038、ARCH-017）。
- 失效原因：与 service-lifecycle 同属服务运行面，按 ADR 0008 两文合并为
  [服务管理中心设计](../../design/service-management.md)（配置与密钥节），本文整篇并入后退役。
