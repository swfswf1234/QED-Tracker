# 下载与清单设计（acquisition-and-inventory）

归档状态：已归档（2026-09-07 自 docs/design/ 移入 history/baselines/）
归档说明：
- 范围：来源协议（search/resolve、libgen_li 发现专用裁决）、数学课程选书要求、
  选择与目录匹配、可靠下载、资源 schema v1、已有文件与完整性、失败语义。
- 失效原因：与 [下载管线设计](../../design/download-pipeline.md)（原 book-download-registration.md）
  职责重叠，按 ADR 0008 整篇并入该文档，同窗退役两文并存。
- 不可变锚点：正文以 2026-09-07 归档时点工作树状态冻结（本批文档含未提交修订，随本轮
  提交定稿；此后仅允许补 Historical 声明、反向关系或修复链接）。
- 当前事实入口：[下载管线设计](../../design/download-pipeline.md)。
设计状态：Historical（2026-09-07 归档）
实现状态：Completed（承载内容已并入 download-pipeline.md）
确认状态：已确认
最后更新：2026-09-07
...
- 目录 target 可选字段 `set_no`（QED-024）：`1`~`4`=中文套 / `en`=英文对照套 / 空=无配套。
  ...
  exercise）；前端按「套」显示课程完成判定（≥2 套 approved 即完成）。`GET /api/v1/catalogs/{id}`
  响应透出该字段。
...
人工评审建议（三态确认时的 `note`）曾保存在 MySQL `qt_resources.review_note`（QED-020，设计已归档：
[人工评审优化](../history/baselines/2026-08-review-round-dedup.md)）。该机制已随 QED-030 退役，
