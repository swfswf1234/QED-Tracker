# 服务与外部接口设计（tracker-service）

归档状态：已归档（2026-09-07 自 docs/design/ 移入 history/baselines/）
归档说明：
- 范围：8901 服务端点与任务模型、Axiom-Flow 消费面、config.py 配置、数据布局、
  qt_resources 资源状态机（QED-012/QED-013/QED-017）、math-qe 书单规格。
- 内容去向：Axiom-Flow 消费面 → [架构 API](../../architecture/api.md)「外部接口」节；
  配置与多项目约定 → [服务管理中心设计](../../design/service-management.md)；
  math-qe 书单规格与 catalog 冻结目录链 → [下载管线设计](../../design/download-pipeline.md)；
  8901 端点契约事实源即 [架构 API](../../architecture/api.md)。
- 失效原因：端点表 16 条中 10 条已随 QED-030 从代码消失，qt_resources 状态机与
  meta/tasks 任务模型被 qt_* 表族与 qt_tasks 取代；文档四职责混杂且大量内容与实现脱节，
  按 ADR 0008 拆散退役。
- 不可变锚点：正文以 2026-09-07 归档时点工作树状态冻结（本批文档含未提交修订，随本轮
  提交定稿；此后仅允许补 Historical 声明、反向关系或修复链接）。
- 当前事实入口：[架构 API](../../architecture/api.md) + [服务管理中心设计](../../design/service-management.md)。
设计状态：Historical（2026-09-07 归档）
实现状态：Completed（服务化轮实现完成；承载内容已分流，见上）
确认状态：已确认
最后更新：2026-09-07
需求方：QED-Engine
关联代码：src/qed_tracker/api/（FastAPI 服务与后台任务层，服务化轮已实现）、src/qed_tracker/db/
（models/engine/schema/repositories，ADR 0006 起无 database.py 与 migrations/）
关联测试：tests/test_api.py（服务化轮新增 API 与任务层测试）、tests/test_db_models.py、
tests/test_schema.py、tests/test_schema_mysql_smoke.py（QED_DB_SMOKE=1 本机冒烟）、8901 真实服务冒烟测试
...
| qed 库 | 共享实例、表命名空间隔离（`qt_*` / `af_*` / 共享 `qed_*`），见 [数据库专用表设计](../architecture/database-private-tables.md)（全库表族总览） |
...
- 直读根 `.env` 的 `QED_*` 变量：唯一密钥 `API_KEY`（百炼；ARCH-017 无厂商别名回退，见
  [模型模式与密钥分置](model-mode-config.md)）、`QED_MODEL`、`QED_AXIOM_URL`
...
- 服务入口：`qed-tracker serve [--host 127.0.0.1] [--port 8901]`；启动先执行
  `ensure_schema()`（ADR 0006：模型即 schema 自愈，替代 `upgrade_database()`），
  MySQL 不可用时仅警告、服务照常启动（登记/任务明确报错）。
...
- 登记顺序：PDF 落盘 → 写资源 JSON → 写 MySQL（已实现：下载任务 progress 70 处经
  ...
  提交同 sha256 幂等复用既有记录）。
- 数据库栈与 schema 演进（ADR 0006，取代「Alembic 迁移」节）：SQLAlchemy 2.0 ORM +
  pymysql；URL 由 QED_DB_* 构造不写死。**模型即 schema**——`db/models.py` 为事实源，
  服务启动 `ensure_schema()` 自愈（缺表补建/列不一致重建/幂等；`qed_llm_calls` 增量补列
  不 DROP）。「ORM 框架旧禁令」治理测试已随数据库层迁移更新。
...
- 若后续需要「人工评估记录」审计视图（谁、何时、何种决策），再加 `assessed_at`/`assessed_by`
  列（模型加列 + ensure_schema 重建，ADR 0006），不影响现有数据。
...
- `QED_TRACKER_LLM_API_KEY` / `DASHSCOPE_API_KEY` / `QWEN_API_KEY` 等厂商别名读取路径退役，
  改读唯一密钥 `API_KEY`（ARCH-017，见[模型模式与密钥分置](model-mode-config.md)）。
