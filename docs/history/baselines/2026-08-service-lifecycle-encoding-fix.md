...
归档说明：
- 范围：中文 Windows 下 tasklist GBK 表头致 `subprocess` 解码失败 → stdout 为 None →
  TypeError 的故障链，与 `_pid_is_alive` 两处修复（errors="replace" + stdout 空值兜底）。
- 归档原因：完成使命——修复已于 2026-08-18 合入 `scripts/qed_tracker_service.py`，
  回归由 `tests/test_service_scripts.py` 持续守护；一次性故障修复，无持续契约价值。
  当前生命周期脚本契约见[服务管理中心设计](../../design/service-management.md)，
  本仓历史正文见[服务生命周期脚本设计（已归档）](./2026-08-service-lifecycle.md)。
