...
并回填决定引用 authors（书行来源：roles 含 textbook 的首行，中文套优先 zh 语言书行）。
...
说明：
- 套1 存在中英两版 textbook 书行（数学分析原理 zh / Principles of Mathematical Analysis en），
  命名取**中文套优先 zh 语言书行**（数学分析原理 / Rudin）。
- 套3 书行 title 已含「（陈纪修）」后缀，`tutorial_name` 检测同名作者后缀去重，
  避免「教程3：数学分析（陈纪修）（陈纪修）」。
- `before.txt` 的 name/textbook_ref 取自 2026-08-20 修正前巡检快照；`updated_at` 以
  `completed_at` 近似（修正前最后一次已知变更），如需精确值可从 MySQL binlog 追溯。
- 书行数据未触碰（与 QED-034 一致，磁盘零触碰）。

## 证据文件

- `before.txt`：修正前三行全行 dump（knowledge + 书行，name/textbook_ref 为修正前真实值）。
