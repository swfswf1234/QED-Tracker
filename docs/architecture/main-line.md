...
- 14 门课程完整清单、先修关系（DAG）、阶段划分与 related_targets 关联见
  [主链路设计](../design/main-line-curriculum.md) 第 1 节。
...
3. **本仓库数据根 = 临时**：下载/登记先落本仓库 `dataset/qed-tracker/`（可删可重建）；
   **人工验收通过后正式移交根仓库 `dataset/qed-tracker/`**。
...
- 教材条目「评价」的粒度与输入方式（自由文本 / 结构化字段 / LLM 预填 + 人工修改）。
- 主链路是否立即建 8901 端点，还是先 CLI/文档跑通 3 门课验证流程。
- 课程体系 `courses/math.json` 的具体字段与课程前置关系（由用户审理命名与顺序）。
- 根仓库 `dataset/qed-tracker/` 的移交动作（复制/移动、登记同步、qed CLI 触发方式）。
