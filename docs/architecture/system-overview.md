...
| `config.py` | 直读根 `.env` 的 `QED_*` 变量（`QWEN_API_KEY`、`QED_MODEL`、`QED_AXIOM_URL`、`QED_TRACKER_PORT`、`QED_PROXY`、`QED_DB_*`）；本地 TOML 与 `QED_TRACKER_*` 已退役。 |
...
8. 写操作全部经后台任务执行（并发上限 2），任务状态落盘并支持轮询；非法状态迁移返回 409。
